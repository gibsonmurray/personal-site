...
function FramerMotion100() {
    ...
    return (
        <div className="center relative h-screen w-screen">
            <div className="absolute top-4 z-10 flex w-full items-center justify-between px-6">
                <motion.a
                    initial={{ opacity: 0 }}
                    animate={{ opacity: backClicked ? 0 : 1 }}
                    transition={{ duration: 0.5, delay: backClicked ? 0 : 0.5 }}
                    onClick={handleBackClicked}
                    className="center cursor-pointer gap-2 text-zinc-400 transition-colors duration-200 hover:text-zinc-700"
                >
                    <motion.div 
                        initial={{ rotate: 90 }}
                        animate={{ rotate: 0 }}
